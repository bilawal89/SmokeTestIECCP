...
public class ValidateSmokeTest extends base {
	...
	public void ValidateLogin() throws IOException, InterruptedException {
		...
		try {
			Assert.assertEquals(TitleHome, "Shop - CCP");
	...
	public void ValidateNotificationPage() throws InterruptedException {
		...
		Assert.assertEquals(Notificationheading_present, true,"Notification heading is not Present in the Notification Page");
		...
		Assert.assertEquals(AllNotification_present, true,"All notification tab is not Present in the Notification Page");
	...
	public void ValidateForgetPswd() throws InterruptedException {
		...
		if(ForgetPswd_present == true){
			...
			Assert.assertEquals(Forgetpswd_Titleact, Forgetpswd_Titlexp,"User is not being able to reach forget password page");
		}
	...
	Assert.assertEquals(validation_present, true,"Validation messages are not appearing in the Forget password Page");
	}
	@Test(priority = 8)
	...
	public void ValidateRegPage() throws InterruptedException {
		...
		Assert.assertEquals(Registeration_Titleact, Registeration_Titleexp,"User is not being able to reach Registeration page");
		...
		if(email_present==true)
		{
			...
			if(VerifyBtn_present==true)
			{
				...
				Assert.assertEquals(ValidationMsg_present, true,"Validation Message is not appearing on the registeration Page");
			}
		}
		}
		}
